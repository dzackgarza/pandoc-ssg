...
1. `content/` — authoring surface. Pages, literal files, static assets, `_data/` control files, opaque sub-sites. Mirrors `dist/` except reserved underscore paths.
2. `pandoc/` — design surface: `defaults/*.yaml`, `templates/*.html`, `partials/`, `filters/*.lua`. Owns layout, math config, component transforms. Defaults files use `${.}`-relative paths only (no machine-local paths).
3. `src/` — the kernel (Bun + TypeScript). Small, boring compiler: scan → classify → validate → route → pandoc invoke → copy → manifest. Modules: `config.ts`, `scan.ts`, `classify.ts`, `schemas.ts` (zod registry, single source of validation truth), `routes.ts`, `pandoc.ts`, `copy.ts`, `manifest.ts`, `build.ts`, `cli.ts`.
