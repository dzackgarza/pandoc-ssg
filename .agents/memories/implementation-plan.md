...
Lower priority: theme/CSS fidelity to minimal-mistakes (islands ship unstyled — `.blog-index`/`.responsive-embed`/`.xournal-embed` have no CSS yet); `toc` component and non-youtube video providers (vimeo) when content needs them; manifest dependency tracking; review CI workflows.
